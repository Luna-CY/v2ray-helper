...
import (
	...
	"errors"
	"fmt"
	"github.com/Luna-CY/v2ray-helper/v1/common/certificate"
	"github.com/Luna-CY/v2ray-helper/v1/common/configurator"
	"github.com/Luna-CY/v2ray-helper/v1/common/database/model"
	"github.com/Luna-CY/v2ray-helper/v1/common/http/code"
	"github.com/Luna-CY/v2ray-helper/v1/common/http/response"
	"github.com/Luna-CY/v2ray-helper/v1/common/logger"
	aria22 "github.com/Luna-CY/v2ray-helper/v1/common/software/aria2"
	caddy2 "github.com/Luna-CY/v2ray-helper/v1/common/software/caddy"
	cloudreve2 "github.com/Luna-CY/v2ray-helper/v1/common/software/cloudreve"
	"github.com/Luna-CY/v2ray-helper/v1/common/software/nginx"
	v2ray2 "github.com/Luna-CY/v2ray-helper/v1/common/software/v2ray"
	util2 "github.com/Luna-CY/v2ray-helper/v1/common/util"
	"github.com/Luna-CY/v2ray-helper/v1/dataservice"
...
type V2rayServerDeployForm struct {
	...
	V2rayConfig v2ray2.Config `json:"v2ray_config"`
}
...
func V2rayServerDeploy(c *gin.Context) {
	...
	if util2.Md5(viper.GetString(configurator.KeyAuthManagementKey)) != body.ManagementKey {
	...
	if !v2ray2.CheckSystem(runtime.GOOS, runtime.GOARCH) {
		response.Response(c, code.BadRequest, fmt.Sprintf("未受支持的系统: %v %v", runtime.GOOS, runtime.GOARCH), nil)

		return
	}

	if err := v2ray2.CheckExists(v2ray2.CmdPath); nil != err {
		if !os.IsExist(err) {
			logger.GetLogger().Errorln(err)
	...
	if body.EnableWebService && (v2ray2.TransportTypeTcp == body.V2rayConfig.TransportType || v2ray2.TransportTypeKcp == body.V2rayConfig.TransportType) {
		response.Response(c, code.BadRequest, "使用TCP或KCP模式时不能开启站点伪装", nil)

		return
	}

	if body.EnableWebService && v2ray2.TransportTypeHttp2 == body.V2rayConfig.TransportType && "/" == body.V2rayConfig.Http2.Path {
		response.Response(c, code.BadRequest, "开启站点伪装时，HTTP2的路径不能为/", nil)

		return
	}

	if v2ray2.TransportTypeHttp2 == body.V2rayConfig.TransportType && !body.UseTls {
	...
	if InstallTypeDefault == body.InstallType || InstallTypeForce == body.InstallType || InstallTypeUpgrade == body.InstallType {
		if err := v2ray2.InstallLastRelease(); nil != err {
			logger.GetLogger().Errorln(err)
	...
	if InstallTypeDefault == body.InstallType || InstallTypeForce == body.InstallType || InstallTypeReConfig == body.InstallType {
		...
		if InstallTypeForce == body.InstallType {
			if vc, err := v2ray2.GetConfig(v2ray2.ConfigPath); nil == err {
		...
		if err := v2ray2.SetConfig(v2ray2.ConfigPath, &body.V2rayConfig); nil != err {
			logger.GetLogger().Errorln(err)
	...
	// 仅在默认安装与重新安装时配置Caddy
	if v2ray2.TransportTypeWebSocket == body.V2rayConfig.TransportType || v2ray2.TransportTypeHttp2 == body.V2rayConfig.TransportType {
		if InstallTypeDefault == body.InstallType || InstallTypeForce == body.InstallType {
			caddyIsInstalled, err := caddy2.IsInstalled()
			if nil != err {
				logger.GetLogger().Errorln(err)
			...
			if !caddyIsInstalled {
				if err := caddy2.InstallLastRelease(); nil != err {
					logger.GetLogger().Errorln(err)
			...
			port := caddy2.PortHttp
			if body.UseTls {
				port = caddy2.PortHttps
			}
			...
			enableHttp2 := false
			if v2ray2.TransportTypeHttp2 == body.V2rayConfig.TransportType {
				enableHttp2 = true
			}

			if err := caddy2.SetConfigToSystem(body.TlsHost, port, body.V2rayConfig.V2rayPort, body.V2rayConfig.WebSocket.Path, body.UseTls, enableCloudreve, enableHttp2); nil != err {
				logger.GetLogger().Errorln(err)
		...
		caddyIsRunning, err := caddy2.IsRunning()
		if nil != err {
			logger.GetLogger().Errorln(err)
	...
	// 处理站点伪装配置
	if InstallTypeDefault == body.InstallType || InstallTypeForce == body.InstallType {
		if body.EnableWebService && cloudreve2.Name == body.WebServiceType {
			if err := cloudreve2.InstallLastRelease(); nil != err {
				logger.GetLogger().Errorln(err)
			...
			if err := cloudreve2.Enable(); nil != err {
				logger.GetLogger().Errorln(err)
			...
			if err := cloudreve2.Start(); nil != err {
				logger.GetLogger().Errorln(err)
			...
			if body.CloudreveConfig.EnableAria2 {
				if err := aria22.InstallToSystem(); nil != err {
					logger.GetLogger().Errorln(err)
				...
				if err := cloudreve2.SetAria2(cloudreve2.DefaultDbPath, "http://127.0.0.1:6800", aria22.DefaultToken, cloudreveAria2RuntimePath); nil != err {
					logger.GetLogger().Errorln(err)
			...
			if body.CloudreveConfig.ResetAdminPassword {
				password, err := cloudreve2.ResetAdminPassword()
				if nil != err {
					logger.GetLogger().Errorln(err)
...
func generateConfig(body V2rayServerDeployForm) error {
	...
	if v2ray2.TransportTypeTcp == body.V2rayConfig.TransportType || v2ray2.TransportTypeKcp == body.V2rayConfig.TransportType {
	...
	for _, client := range body.V2rayConfig.Clients {
		...
		endpoint.Deleted = util2.NewFalsePtr()
...
func stopAllService() error {
	...
	v2rayIsRunning, err := v2ray2.IsRunning()
	if nil != err {
		return err
	}
	...
	if v2rayIsRunning {
		if err := v2ray2.Stop(); nil != err {
			return err
		}
	}

	cloudreveIsRunning, err := cloudreve2.IsRunning()
	if nil != err {
		return err
	}

	if cloudreveIsRunning {
		if err := cloudreve2.Stop(); nil != err {
			return err
		}
	}

	if err := cloudreve2.Disable(); nil != err {
		return err
	}

	aria2IsRunning, err := aria22.IsRunning()
	if nil != err {
		return err
	}

	if aria2IsRunning {
		if err := aria22.Stop(); nil != err {
			return err
		}
	}

	if err := aria22.Disable(); nil != err {
		return err
	}
